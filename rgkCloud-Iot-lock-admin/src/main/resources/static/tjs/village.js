...
function initGrid(){
	$("#grid-table-village").jqGrid({
			url : 'village/list?jqGridID=JQGrid1',
			datatype : "json",
			height : '100%',
			colNames : ['id','园区','经度','纬度', '创建日期','操作'],
			colModel : [
					{
						name : 'id',
						index : 'id',
			...
			loadComplete : function() {
				...
				// 隐藏滚动条
				$("#grid-table-village").closest(".ui-jqgrid-bdiv").css({ 'overflow-y' : 'hidden','overflow-x':'hidden' });
			},

			editurl : "village/save",// nothing is saved
			caption : "园区列表"
		});
}
...
function navButtons(){
	jQuery("#grid-table-village").jqGrid('navGrid',"#grid-pager-village",
			{ // navbar options
				edit : false,
...
$('.page-content-area').ace_ajax(
				...
				function() {
					jQuery(function($) {
						var grid_selector = "#grid-table-village";
						var pager_selector = "#grid-pager-village";
