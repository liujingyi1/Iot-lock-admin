...
function initGrid(){
	$("#grid-table-building").jqGrid({
			url : 'building/list?jqGridID=JQGrid1',
			datatype : "json",
			height : '100%',
			autoScroll: true, 
			colModel : [{
						...
						edittype:"select",
						formatter: 'select',
						editoptions:{value:getVillages(),formatter:'select'},
					},{
						label : '楼栋',
						name : 'alias',
						editable : true
			...
			editurl : "building/save",// nothing is saved
			caption : "楼栋列表"
		});
}
...
function navButtons(){
	jQuery("#grid-table-building").jqGrid('navGrid',"#grid-pager-building",
			{ // navbar options
				edit : false,
...
$('.page-content-area').ace_ajax(
				...
				function() {
					jQuery(function($) {
						var grid_selector = "#grid-table-building";
						var pager_selector = "#grid-pager-building";
