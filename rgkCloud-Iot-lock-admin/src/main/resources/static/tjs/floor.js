...
function initGrid(){
	...
	var tmp1 = 0;
	var buildingTmp="";
	$("#grid-table-floor").jqGrid({
			url : 'floor/list?jqGridID=JQGrid1', 
			datatype : "json",
			height : '100%',
			autoScroll: true, 
			colModel : [
			...
			loadComplete : function(xhr) {
				...
				//隐藏滚动条
				$("#grid-table-floor").closest(".ui-jqgrid-bdiv").css({ 'overflow-y' : 'hidden','overflow-x':'hidden' });
			},
			editurl : "floor/save",// nothing is saved
			caption : "楼层列表"
		});
}
...
function getBuildingByVillageId() {
	var flag = 0;
	var id = $('#grid-table-floor').jqGrid('getGridParam', 'selrow');
...
function navButtons(){
	jQuery("#grid-table-floor").jqGrid('navGrid',"#grid-pager-floor",
			{ // navbar options
				edit : false,
